// https://kdb.tsukuba.ac.jp/syllabi/theme/default/newportal/CLEditor1_4_4/jquery.cleditor.js
...
(function ($) {
  ...
  cleditor = function (area, options) {
    ...
    var $toolbar = (editor.$toolbar = $(DIV_TAG)
      ...
      .addClass("clearfix") // add by taka
      .width(options.width) // add by taka
      .appendTo($main));
    ...
    $.each(options.controls.split(" "), function (idx, buttonName) {
      ...
      // Button
        ...
        if (ie) $buttonDiv.attr(UNSELECTABLE, "on");
  ...
  function buttonClick(e) {
    ...
    // Check if disabled
    if (editor.disabled || $(buttonDiv).attr(DISABLED) === DISABLED) return;
    ...
    // Check for rich text mode
      ...
      // Print
      else if (buttonName === "print") editor.$frame[0].contentWindow.print();
      // All other buttons
      else if (
        !execCommand(editor, data.command, data.value, data.useCSS, buttonDiv)
      )
        return false;
    }
  ...
  function popupClick(e) {
    ...
    if (popup === popups.msg || $(popup).hasClass(PROMPT_CLASS)) return;
    ...
    if (buttonName === "font")
      ...
      if (target.tagName.toUpperCase() === "DIV") target = target.children[0];
      value = target.innerHTML;
    } else if (buttonName === "style") value = "<" + target.tagName + ">";
    else if (buttonName === "color") value = hex(target.style.backgroundColor);
    ...
    if (button.popupClick && button.popupClick(e, data) === false) return;
  ...
  function createPopup(
    ...
    // Custom popup
    if (popupContent) $popup.html(popupContent);
    // Color
    else if (popupName === "color") {
      var colors = options.colors.split(" ");
      if (colors.length < 10) $popup.width("auto");
    ...
    if (!popupTypeClass && !popupContent) popupTypeClass = LIST_CLASS;
  ...
  function hex(s) {
    ...
    var c = s.split("");
    if (s.length === 4) return "#" + c[1] + c[1] + c[2] + c[2] + c[3] + c[3];
  ...
  function refresh(editor) {
    ...
    if (editor.$frame) editor.$frame.remove();
    ...
    // add by taka
    var style = "";
    for (i = 0; i < options.docCSSFile.length; i++) {
      style +=
        '<link rel="stylesheet" type="text/css" href="' +
        options.docCSSFile[i] +
        '" />';
    }
    ...
    if (ie || iege11) {
      ...
      $doc.bind(
        ...
        function (e) {
          ...
          if (e.type === "beforedeactivate") editor.inactive = true;
          ...
          // Save the selection when the editor is active
            ...
            if (!editor.range) editor.range = [];
            editor.range.unshift(getRange(editor));

            // We only need the last 2 selections
            while (editor.range.length > 2) editor.range.pop();
          }
        }
      );
    ...
    $(function () {
      ...
      // Resize the toolbar
      // mod by taka toolbarの手動リサイズ変更（ダイアログ表示で正しい計算ができないので）
      //var hgt = $group.offset().top + $group.height() - $toolbar.offset().top + 1;
      //$toolbar.height(hgt);
      var w = 0;
      ...
      var hgt = Math.ceil(w / options.width) * 26;

      // Resize the iframe
      hgt =
        (/%/.test("" + options.height)
          ? $main.height()
          : parseInt(options.height, 10)) - hgt;
  ...
  function refreshButtons(editor) {
    ...
    $.each(editor.$toolbar.find("." + BUTTON_CLASS), function (idx, elem) {
      ...
      // Determine the state
      if (editor.disabled) enabled = false;
        ...
        if (ie && command === "hilitecolor") command = "backcolor";
      ...
      if (button.name == "source") {
        ...
        var scriptflg = html.search(/<(?=\/?script)/gi) != -1;
        if (scriptflg) {
          enabled = false;
        }
      }
  ...
  function restoreRange(editor) {
    if (editor.range) {
      if (ie) editor.range[0].select();
      else if (iege11) getSelection(editor).addRange(editor.range[0]);
    }
  }
  ...
  function updateFrame(editor, checkForChange) {
    ...
    // add by taka
    var xssCode = code.replace(/<(?=\/?script)/gi, "&lt;");
    ...
    if (updateFrameCallback) {
      ...
      var sum = checksum(xssCode);
      if (checkForChange && editor.areaChecksum === sum) return;
    ...
    if (options.updateTextArea) editor.frameChecksum = checksum(html);
  ...
  function updateTextArea(editor, checkForChange) {
    ...
    if (updateTextAreaCallback) {
      ...
      if (checkForChange && editor.frameChecksum === sum) return;
    ...
    if (options.updateFrame) editor.areaChecksum = checksum(code);
